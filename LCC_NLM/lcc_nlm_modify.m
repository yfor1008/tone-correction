function [lcc, mask] = lcc_nlm_modify(gray, radius)
% lcc_nlm_modify - Local Color Correction Using Non-Linear Masking
%
% input:
%   - gray: H*W, 灰度图像, uint8
%   - radius: int, 滤波窗口半径
...
%   - 对算法进行调整:
%   - 1. 滤波得到mask
%   - 2. 使用公式进行 gamma 矫正, 公式更改为: Oxy=255(Ixy/255)^(2^((Mxy-127)/128))
...
lcc = 255 * (gray / 255) .^ (2 .^((mask1 - 127) / 128));
